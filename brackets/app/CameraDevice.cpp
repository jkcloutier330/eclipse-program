...
void CameraDevice::capture_image(int shuttertime) const
{
    tout << "Capture image...";
    tout << "Shutter down...";
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(shuttertime));
    tout << "Shutter up\n";
   ...
   /*  load_properties();
	...
	for(int i = 0; i<retries; i++) { 
		...
		if (1 != m_prop.still_capture_mode.writable) {
			...
			tout << ".";
			if (i==retries-1) {	
				tout << "Continuous shutter is not writable\n";			
				return;
			}
			usleep(waittime);		
		} else {
			// tout << "Shutter Speed is writable\n";
			break;
		}
	}	
	get_still_capture_mode();
...
void CameraDevice::EVBracket(int option)
{
	tout << "Bracketing";	
	...
	SDK::CrDriveMode intendedMode = SDK::CrDriveMode::CrDrive_Continuous_Bracket_10Ev_9pics;
	switch (option){
		case 0:			
			intendedMode = SDK::CrDriveMode::CrDrive_Continuous_Bracket_10Ev_9pics;
			break;
		case 1:			
	...
	SDK::CrDeviceProperty mode;
	mode.SetCode(SDK::CrDevicePropertyCode::CrDeviceProperty_DriveMode);
	mode.SetCurrentValue(intendedMode);
	mode.SetValueType(SDK::CrDataType::CrDataType_UInt32Array);
	for(int i = 0; i<retries; i++) { 
		load_properties();
		if (m_prop.still_capture_mode.current != intendedMode) {
			auto err_still_capture_mode = SDK::SetDeviceProperty(m_device_handle, &mode);
			if (CR_FAILED(err_still_capture_mode)) {
				tout << "Still Capture Mode setting FAILED\n";
				return;
			...
			tout << ".";
			if (i==retries-1) {	
				tout << "capture mode is not writable\n";			
				return;
			}
...
void CameraDevice::set_shutter_speed(int setting)
{	
	tout << "Setting shutter speed";	
	...
	for(int i = 0; i<retries; i++) { 
		...
		if ((1 != m_prop.shutter_speed.writable) or  (values.size() == 0)) {
			// Not a settable property
			tout << ".";
			if (i==retries-1) {	
	...
	load_properties();
	if (m_prop.shutter_speed.current == values[setting]) {
		tout << ", Shutter Speed already set\n";	
		return;
	}		
	
	for(int i = 0; i<retries; i++) { 
		load_properties();		
		// this->get_shutter_speed();	
		if (m_prop.shutter_speed.current != values[setting]) {
			// Not a settable property
			tout << ".";
			
			if (i==retries-1) {	
				tout << "Shutter Speed couldn't be set\n";			
				return;
			}			
	
			SDK::CrDeviceProperty prop;
			prop.SetCode(SDK::CrDevicePropertyCode::CrDeviceProperty_ShutterSpeed);
			prop.SetCurrentValue(values[setting]);
			prop.SetValueType(SDK::CrDataType::CrDataType_UInt32Array);
...
void CameraDevice::noPCDownload()
{    
    SCRSDK::CrDeviceProperty prop;
    prop.SetCode(SDK::CrDevicePropertyCode::CrDeviceProperty_StillImageStoreDestination); // Specify the code of the device property you want to update
    prop.SetValueType(SDK::CrDataType::CrDataType_UInt16Array); // Specify the type of the device property you want to update
    prop.SetCurrentValue((CrInt64u) 2 );
